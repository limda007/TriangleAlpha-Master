...
deploy-slave: ## 构建并部署被控端到 VM（生产版，无控制台）
	...
	scp src/common/__init__.py src/common/protocol.py src/common/models.py src/common/app_version.py $(VM_HOST):"$(VM_BUILD_DIR)/src/common/"
	scp src/common_app_version.py $(VM_HOST):"$(VM_BUILD_DIR)/src/"
	...
	@echo ">>> 安装依赖..."
	ssh $(VM_HOST) "$(VM_PYTHON) -m pip install pydantic --quiet"
	@echo ">>> 在 VM 上构建..."
