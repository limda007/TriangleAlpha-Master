...
class TestLocalIpcReceiver:
    ...
    def test_update_and_snapshot(self):
        ...
        assert data["account"] == "u1"
        assert age < 1.0

    def test_snapshot_age_increases(self):
        receiver = LocalIpcReceiver()
        receiver._last_sync = time.monotonic() - 10
        receiver._data = {"account": "x", "level": "1", "jinbi": "0", "status_text": "", "elapsed": "0"}
        _, age = receiver.snapshot()
        assert age >= 9.5

    def test_snapshot_returns_copy(self):
        """snapshot 返回的是副本，不影响内部状态。"""
        receiver = LocalIpcReceiver()
        receiver._on_message({
            "account": "u1", "level": "10",
            "jinbi": "500", "status_text": "ok", "elapsed": "30",
        })
        data1, _ = receiver.snapshot()
        data1["account"] = "modified"
        data2, _ = receiver.snapshot()
        assert data2["account"] == "u1"
